...
function marquee() {
  //marquee_wrap을 전부 찾아서 각각 독립적으로 흐르게 만든다
...
function splitWords(selector, wordClass) {
  ...
  document.querySelectorAll(selector).forEach((textEl) => {
    //textEl 요소 안의 텍스트를 가져와서 앞뒤 공백을 제거한뒤에 공백 기준으로 나누어 배열로 만드는 코드
...
photos.forEach((photo, index) => {
  //사진이 등장하기 직전에 레이어 순서와 glitch 클래스를 조정
  pinTl.add(() => {
    //뒤에 나오는 사진이 앞쪽에 쌓이도록 z-index 올림
...
// 가로 스크롤 섹션에서 실제로 옆으로 움직여야 하는 거리를 계산
